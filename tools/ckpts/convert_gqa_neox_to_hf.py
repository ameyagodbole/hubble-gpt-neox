...
MODEL_KEYS = {
    ...
    "llama": {
        "new": {
            ...
            },  # No biases in RowParallelLinear layers
            "NORM_KEYS": {
...
def load_partitions(
    ...
    if sequential:
        ...
        print(f"Loading model states for layer {layer_idx} from {input_checkpoint_path} | Sequential")
    else:
        filename_format = f"layer_{layer_idx:02}-model_{{i:02}}-model_states.pt"
        print(f"Loading model states for layer {layer_idx} from {input_checkpoint_path} | Pipeline")
...
def get_state(
    ...
    if sequential:
        ...
        print(f"In get_state sequential | key: {key}")
        ...
        key = key
        print(f"In get_state pipeline | key: {key}")
        # print(f"State Dicts: {state_dicts}")
        # print(f"State Dict Keys: {[state_dict[key] for state_dict in state_dicts]}")
        print(f"Returning {[state_dict[key] for state_dict in state_dicts]}")
        return [state_dict[key] for state_dict in state_dicts]
...
def create_config(neox_config, architecture="neox", is_rm=False, pad_token_id=-1):
    ...
    if architecture == "mistral" or architecture == "llama":
        ...
        print(f"Using intermediate size: {args['intermediate_size']}")
        print(f"Using num_key_value_heads: {args['num_key_value_heads']}")
        print(f"Using hidden_act: {args['hidden_act']}")
        print(f"Using rms_norm_eps: {args['rms_norm_eps']}")
        print(f"Using bos_token_id: {args['bos_token_id']}")
        print(f"Using eos_token_id: {args['eos_token_id']}")
        print(f"Using rope_theta: {args['rope_theta']}")
        ...
        if architecture == "mistral":
            ...
            args.update(
                {
                    # NeoX library defaults to using bias in attention and MLPs
                    "attention_bias": get_key(
                        neox_config, "use_bias_in_attn_linear", True
                    ),
                    "mlp_bias": get_key(
                        neox_config, "use_bias_in_mlp", True
                    ),
                }
            )
            print(f"Using attention_bias: {args['attention_bias']}")
            print(f"Using mlp_bias: {args['mlp_bias']}")
            hf_config = LlamaConfig(**args)
            print(f"Using Args: {args}")
            print(f"Using hf_config: {hf_config}")
    else:
...
def reshard_and_split_qkv(
    ...
    for key, hf_keys in param_mapping.items():
        ...
        print(f"Attention layer {layer_idx}: q.shape={q.shape}, k.shape={k.shape}, v.shape={v.shape}")
...
def get_mlp_naming_convention(loaded_tp_ranks, layer_idx, sequential):
    ...
    if any(
        ...
        print("MLP naming convention: new")
        return "new"
    elif any(
        [
            [
        ...
        print("MLP naming convention: legacy")
        return "legacy"
    else:
...
def convert(
    ...
    print(f"Created HF config: {hf_config}")
    ...
    print(f"Created HF model: {hf_model}")
    print(f"Created HF transformer: {hf_transformer}")
    ...
    if precision == "auto":
        ...
        if isinstance(fp16, dict) and fp16.get("enabled", False):
            hf_model.half()
            print("Saving weights in fp16 precision...")
        elif isinstance(bf16, dict) and bf16.get("enabled", False):
            hf_model.to(dtype=torch.bfloat16)
            print("Saving weights in bf16 precision...")
        else:
            hf_model.to(dtype=torch.float)
    ...
    print(f"Model precision: {hf_model.dtype}")

    mp_partitions = get_key(loaded_config, "model-parallel-size")
    
    print(f"MP Partitions: {mp_partitions}")
    ...
    if architecture == "neox":
        ...
        print(f"Embedding layer: {embed_in}")
    print(f"Starting loading of Embedding layer")
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        for key, hf_key in ARCH["COLUMN_PARALLEL_BIAS_KEYS"].items():
            if type(hf_key) == list:
                # Llama magic - split the weight into two parts for the gate and up proj
                states = [
                    torch.chunk(state, chunks=2, dim=0)
    ...
    final_layer_norm.load_state_dict(norm_state_dict, strict=True)
...
def main(input_args=None, overwrite_values=None):
    ...
    if not args.no_save_tokenizer:
        ...
        if args.vocab_is_hf_tokenizer:
            print(f"In Vocab is HF Tokenizer")
            from transformers import AutoTokenizer
            print("Vocab File Path: ", os.path.dirname(get_key(loaded_config, "vocab-file")))
